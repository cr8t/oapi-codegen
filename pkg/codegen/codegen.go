...
func WriteSingleFile(opts Configuration, generated map[string]string) (string, error) {
	...
	if _, err = w.WriteString(generated[GenConstants]); err != nil {
		return "", fmt.Errorf("error writing constants: %w", err)
	}

	if _, err = w.WriteString(generated[GenTypes]); err != nil {
		return "", fmt.Errorf("error writing type definitions: %w", err)
	}

	if opts.Generate.Client {
		if _, err = w.WriteString(generated[Client]); err != nil {
			return "", fmt.Errorf("error writing client: %w", err)
		}
		if _, err = w.WriteString(generated[ClientWithResponses]); err != nil {
			return "", fmt.Errorf("error writing client: %w", err)
		}
	}

	if opts.Generate.EchoServer {
		if _, err = w.WriteString(generated[EchoServer]); err != nil {
			return "", fmt.Errorf("error writing server path handlers: %w", err)
		}
	}

	if opts.Generate.ChiServer {
		if _, err = w.WriteString(generated[ChiServer]); err != nil {
			return "", fmt.Errorf("error writing server path handlers: %w", err)
		}
	}

	if opts.Generate.GinServer {
		if _, err = w.WriteString(generated[GinServer]); err != nil {
			return "", fmt.Errorf("error writing server path handlers: %w", err)
		}
	}

	if opts.Generate.GorillaServer {
		if _, err = w.WriteString(generated[GorillaServer]); err != nil {
			return "", fmt.Errorf("error writing server path handlers: %w", err)
		}
	}

	if opts.Generate.Strict {
		if _, err = w.WriteString(generated[Strict]); err != nil {
			return "", fmt.Errorf("error writing server path handlers: %w", err)
		}
	}

	if opts.Generate.EmbeddedSpec {
		if _, err = w.WriteString(generated[EmbeddedSpec]); err != nil {
			return "", fmt.Errorf("error writing inlined spec: %w", err)
		}
	}

	if opts.Generate.KitClient {
		if _, err = w.WriteString(generated[KitClient]); err != nil {
			return "", fmt.Errorf("error writing go-kit client: %w", err)
		}
	}

	if opts.Generate.KitEndpoints {
		if _, err = w.WriteString(generated[KitEndpoints]); err != nil {
			return "", fmt.Errorf("error writing go-kit endpoints: %w", err)
		}
	}

	if opts.Generate.KitLogger {
		if _, err = w.WriteString(generated[KitLogger]); err != nil {
			return "", fmt.Errorf("error writing go-kit logger: %w", err)
		}
	}

	if opts.Generate.KitMiddleware {
		if _, err = w.WriteString(generated[KitMiddlewareChaos]); err != nil {
			return "", fmt.Errorf("error writing go-kit middleware chaos: %w", err)
		}

		if _, err = w.WriteString(generated[KitMiddlewareLogging]); err != nil {
			return "", fmt.Errorf("error writing go-kit middleware logging: %w", err)
		}

		if _, err = w.WriteString(generated[KitMiddlewareMetrics]); err != nil {
			return "", fmt.Errorf("error writing go-kit middleware metrics: %w", err)
		}

		if _, err = w.WriteString(generated[KitMiddlewareTracing]); err != nil {
			return "", fmt.Errorf("error writing go-kit middleware tracing: %w", err)
		}
	}

	if opts.Generate.KitService {
		if _, err = w.WriteString(generated[KitService]); err != nil {
			return "", fmt.Errorf("error writing go-kit service stub: %w", err)
		}
	}

	if opts.Generate.KitServiceStub {
		if _, err = w.WriteString(generated[KitServiceStub]); err != nil {
			return "", fmt.Errorf("error writing go-kit server: %w", err)
		}
	}

	if opts.Generate.KitTransport {
		if _, err = w.WriteString(generated[KitTransport]); err != nil {
...
func WriteMultiFile(opts Configuration, generated map[string]string) (string, error) {
	...
	for name, w := range writers {
		if generated[name] != "" {
			if _, err = w.WriteString(generated[GenImports]); err != nil {
				return "", fmt.Errorf("error writing imports: %w", err)
			}
		}
	}

	if _, err = writers[GenConstants].WriteString(generated[GenConstants]); err != nil {
		return "", fmt.Errorf("error writing constants: %w", err)
	}

	if _, err = writers[GenTypes].WriteString(generated[GenTypes]); err != nil {
		return "", fmt.Errorf("error writing type definitions: %w", err)
	}

	if opts.Generate.Client {
		if _, err = writers[Client].WriteString(generated[Client]); err != nil {
			return "", fmt.Errorf("error writing client: %w", err)
		}

		if _, err = writers[Client].WriteString(generated[ClientWithResponses]); err != nil {
			return "", fmt.Errorf("error writing client: %w", err)
		}
	}

	if opts.Generate.EchoServer {
		if _, err = writers[EchoServer].WriteString(generated[EchoServer]); err != nil {
			return "", fmt.Errorf("error writing server path handlers: %w", err)
		}
	}

	if opts.Generate.ChiServer {
		if _, err = writers[ChiServer].WriteString(generated[ChiServer]); err != nil {
			return "", fmt.Errorf("error writing server path handlers: %w", err)
		}
	}

	if opts.Generate.GinServer {
		if _, err = writers[GinServer].WriteString(generated[GinServer]); err != nil {
			return "", fmt.Errorf("error writing server path handlers: %w", err)
		}
	}

	if opts.Generate.GorillaServer {
		if _, err = writers[GinServer].WriteString(generated[GorillaServer]); err != nil {
			return "", fmt.Errorf("error writing server path handlers: %w", err)
		}
	}

	if opts.Generate.Strict {
		if _, err = writers[Strict].WriteString(generated[Strict]); err != nil {
			return "", fmt.Errorf("error writing server path handlers: %w", err)
		}
	}

	if opts.Generate.EmbeddedSpec {
		if _, err = writers[EmbeddedSpec].WriteString(generated[EmbeddedSpec]); err != nil {
			return "", fmt.Errorf("error writing inlined spec: %w", err)
		}
	}

	if opts.Generate.KitClient {
		if _, err = writers[KitClient].WriteString(generated[KitClient]); err != nil {
			return "", fmt.Errorf("error writing inlined spec: %w", err)
		}
	...
	if opts.Generate.KitEndpoints {
		if _, err = writers[KitEndpoints].WriteString(generated[KitEndpoints]); err != nil {
			return "", fmt.Errorf("error writing kit endpoints: %w", err)
		}
	}

	if opts.Generate.KitLogger {
		if _, err = writers[KitLogger].WriteString(generated[KitLogger]); err != nil {
			return "", fmt.Errorf("error writing kit logger: %w", err)
		}
	}

	if opts.Generate.KitMiddleware {
		if _, err = writers[KitMiddlewareChaos].WriteString(generated[KitMiddlewareChaos]); err != nil {
			return "", fmt.Errorf("error writing kit middleware chaos: %w", err)
		}

		if _, err = writers[KitMiddlewareLogging].WriteString(generated[KitMiddlewareLogging]); err != nil {
			return "", fmt.Errorf("error writing kit middleware logging: %w", err)
		}

		if _, err = writers[KitMiddlewareMetrics].WriteString(generated[KitMiddlewareMetrics]); err != nil {
			return "", fmt.Errorf("error writing kit middleware metrics: %w", err)
		}

		if _, err = writers[KitMiddlewareTracing].WriteString(generated[KitMiddlewareTracing]); err != nil {
			return "", fmt.Errorf("error writing kit middleware tracing: %w", err)
		}
	}

	if opts.Generate.KitService {
		if _, err = writers[KitService].WriteString(generated[KitService]); err != nil {
			return "", fmt.Errorf("error writing inlined spec: %w", err)
		}
	}

	if opts.Generate.KitServiceStub {
		if _, err = writers[KitServiceStub].WriteString(generated[KitServiceStub]); err != nil {
			return "", fmt.Errorf("error writing inlined spec: %w", err)
		}
	...
	if opts.Generate.KitTransport {
		if _, err = writers[KitTransport].WriteString(generated[KitTransport]); err != nil {
