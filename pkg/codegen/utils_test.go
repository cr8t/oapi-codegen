...
func TestStringOps(t *testing.T) {
	...
	assert.Equal(t, "WordWordWORDWordWordWordWordWordWordWordWordWordWord", ToPascalCase("word.word-WORD+Word_word~word(Word)Word{Word}Word[Word]Word:Word;"), "Camel case conversion failed")

	// Make sure numbers don't interact in a funny way.
	assert.Equal(t, "Number1234", ToPascalCase("number-1234"), "Number Camelcasing not working.")
}
